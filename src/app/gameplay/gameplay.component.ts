...
export class GameplayComponent implements OnInit {
  ...
  sample: Howl = new Howl({
    src: [
      "https://p.scdn.co/mp3-preview/f60beaf9fa2e73a0c1946c2402e0996270920f78?cid=74f434552d40467782bc1bc64b12b2e9",
    ],
    format: ["mp3"],
  });
  ...
  loadQuestionData() {
    ...
    this.tracksService.currentQuestion.subscribe(
      (question) => (this.currentQuestion = question)
    );
    console.log(this.currentQuestion);

    this.firstAlbumImageUrl =
      this.currentQuestion.track1.track.album.images[0]?.url;
    this.secondAlbumImageUrl =
      this.currentQuestion.track2.track.album.images[0]?.url;
  ...
  chooseAlbum(albumNumber: number) {
    ...
    if (this.questionNumber <= totalQuestions) {
      ...
      this.game.numberOfQuestions = 10;
      this.game.correctAnswers = this.correctAnswers;
      this.game.incorrectAnswers = this.incorrectAnswers;
      this.game.finalScore = this.currentScore;

      this.userService.updateCurrentGame(this.game);
  ...
  calculateCurrentScore(): number {
    return this.selectedDifficulty === "hard"
      ? this.correctAnswers * 300
      : this.selectedDifficulty === "medium"
      ? this.correctAnswers * 200
